...
@section('maincontent')
    ...
    <div class="contentbar">
                            ...
                            <div class="content-block box-body content-block-two" style="padding:5px">
                                <form action="{{ route('currency.exchanges.save') }}" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <small>
                                            <a target="__blank" title="Get your keys from here" class="text-info float-right"
                                                href="https://openexchangerates.org/signup/free"><i
                                                    class="fa fa-key"></i>{{ __('Get Your OPEN EXCHANGE RATE KEY From Here') }}
                                            </a>
                                        </small>
                                        <hr>

                                        <input required id="OPEN_EXCHANGE_RATE_KEY"
                                            value="{{ env('OPEN_EXCHANGE_RATE_KEY') }}" name="OPEN_EXCHANGE_RATE_KEY"
                                            type="text" class="form-control" placeholder="enter OPEN EXCHANGE RATE KEY">
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card m-b-30">
                    <div class="widgetbar m-3">
                        @can('currency.create')
                            <a href="" data-toggle="modal" data-target="#addcurrency"
                                class="float-right btn btn-primary-rgba mr-1"> <i
                                    class="feather icon-plus mr-2"></i>{{ __('Add new currency') }}</a>
                        @endcan
                        @can('currency.edit')
                            <a href="#" class="float-right btn btn-success-rgba mr-1 ml-1 updateRateBtn"><span
                                    id="buttontext">{{ __('Update Currency Rate') }}</span></a>
                        @endcan
                    </div>
                    <div class="card-body">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="{{ route('currency.store') }}" method="post">
                                        @csrf
                                        <div class="form-group">
                                            <label for="my-input">{{ __('Currency') }} (ISO code 3): <span
                                        ...
                                        <button type="submit" class="btn btn-primary"><i
                                                class="fa fa-check-circle"></i>{{ __('Create') }} </button>
                                    </form>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#currency').DataTable({
                language: {
                    searchPlaceholder: "Search currency here"
                },
