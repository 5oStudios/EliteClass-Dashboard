<div class="dropdown">
    ...
    <div class="dropdown-menu" aria-labelledby="CustomdropdownMenuButton1">
        @can('childcategories.edit')
            <a href="{{ route('childcategory.edit', $id) }}" class="dropdown-item"><i
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-muted">
                    {{ __('Do you really want to delete this ') }} <b>{{ __('major') }}</b>
                    {{ __('? This process cannot be undone') }}
                </p>
            </div>
            <div class="modal-footer">
                <form method="post" action="{{ action('ChildcategoryController@destroy', $id) }}" class="float-right">
                    {{ csrf_field() }}
                    {{ method_field('DELETE') }}
