...
class WalletController extends Controller {
    ...
    public function walletTopUp(Request $request) {
        ...
        $request->validate([
            ...
            "amount.min"=>__("Amount is invalid"),
        ...
        return response()->json(array('success' => __('Topup success')));
    }

    public function getWallet(Request $request, $t = 'Credit') {

        $user = Auth::user();
        $b = WalletTransactions::where(['user_id' => $user->id, 'type' => $t])
                 ->when($t == 'Debit',function($q){
                    $q->where('payment_method','Wallet');
                })
                ->sum('total_amount');
        $Wallet = WalletTransactions::
                select('id', 'total_amount', 'payment_method', 'currency', 'detail', DB::raw('date(created_at) as date'))
                ->where(['user_id' => $user->id, 'type' => $t])
                ->when($t == 'Debit',function($q){
                    $q->where('payment_method','Wallet');
                })
                ->latest()
        ...
        return response()->json(array(
                    'corrent_balance' => $user->wallet->balance,
                    'total' => $b,
                    'history' => $Wallet->groupBy('date')
                        ), 200);
    }
