...
Route::get('send-mail/{email}', function ($email) {
    ...
    try {
        ...
        return response()->json("Email is not Sent.");
    }
});
...
Route::get('/test-push/{player_id}/{msg}', function ($player_id, $msg) {
    $fields['include_player_ids'] = [$player_id];
    $notificationMsg = 'Hello!! This is Nouman Sarwar!';
...
// KNET payment confirmation page route
// Route::get('/user/knet/payment/detail', 'Api\KnetPaymentController@knetPaymentDetail')->name('knet.payment.detail');
...
Route::middleware('basicAuth')->group(function () {
    Route::get('/uptime-status', 'DatabaseController@uptime');
});
...
Route::middleware(['web', 'switch_languages'])->group(function () {
    ...
    Route::post('bigblue/api/recordingcallback', 'BigBlueController@recordingReady');
    ...
    Route::middleware(['IsInstalled', 'ip_block'])->group(function () {
        ...
        Route::get('email/resend', 'Auth\VerificationController@resend')->name('verification.email.resend');

        Route::get('language-switch/{local}', 'LanguageSwitchController@languageSwitch')->name('languageSwitch');

        // Route::middleware(['auth', 'maintanance_mode'])->group(function () {
        Route::middleware(['IsInstalled', 'auth', 'twofactor'])->group(function () {
            ...
            Route::get('typecategories/status', 'TypeCategoryController@status')->name('typecategories.status');
            Route::post('typecategories/bulk_delete', 'TypeCategoryController@bulk_delete')->name('typecategories.bulk_delete');
            ...
            Route::prefix('user')->group(function () {
                ...
                Route::get('/zaki', 'UserController@viewAllZakiUser')->name('user.zakiindex');
            ...
            Route::prefix('alluser')->group(function () {
                Route::get('/', 'AlluserController@viewAllUser')->name('alluser.index');
            ...
            Route::post('course-installment', 'CourseController@InstallmentStore')->name('courseinstallment.store');
            Route::delete('installment-delete/{id}', 'CourseController@InstallmentDelete')->name('courseinstallment.delete'); //abandoned
            Route::post('installment-delete-bulk', 'CourseController@Installment_bulk_delete')->name('courseinstallment.delete.bulk'); //abandoned
            Route::get('/quickupdate-course-installment/{id}', 'CourseController@courceInstallmentStatus')->name('installment.quick.status'); //abandoned
            ...
            Route::post('courseclasses/copy/', 'CoursechapterController@copyCourseClasses')->name('course.classes.copy');
            Route::post('coursechapter/duplicate/{id}', 'CoursechapterController@duplicate')->name('course.chapter.duplicate');
            ...
            Route::get('uploadvideotovimeo', 'CourseclassController@uploadVideoToVimeo')->name('courseclass.uploadVideoToVimeo');
            // Route::get('uploadvideotobunnycdnzk','CourseclassController@uploadVideoToBunnyCDNzk')->name('courseclass.uploadVideoToBunnyCDNzk');
            Route::get('uploadvideotobunnycdn', 'CourseclassController@uploadVideoToBunnyCDN')->name('courseclass.uploadVideoToBunnyCDN');
            Route::get('courseclass/create/{id}/{chap}', 'CourseclassController@create')->name('courseclass.add');
            Route::get('chapterclasses/{id}', 'CourseclassController@index')->name('chapterclasses');
            ...
            Route::post('bundle/installment', 'BundleCourseController@storeInstallments')->name('bundleinstallment.store');
            ...
            Route::prefix('bigblue')->group(function () {
                ...
                Route::get('meeting/create', 'BigBlueController@create')->name('bbl.create');
                ...
                Route::get('meeting/user/enrolled/{meetingid}', 'BigBlueController@enrolledUser')->name('bbl.enrolled');
            ...
            Route::resource('sessions', 'OfflineSessionController', [
                'as' => 'offline',
            ]);

            Route::get('session/user/enrolled/{sessionid}', 'OfflineSessionController@enrolledUser')->name('session.enrolled');

            Route::resource('coupon', 'CouponController');
            Route::get('coupon/bulk/create', 'CouponController@bulkCreate')->name('coupon.bulk.create');
            Route::post('coupon/bulk', 'CouponController@bulkStore')->name('coupon.bulk.store');
            ...
            Route::get('/transactions', 'WalletSettingController@transactions')->name('transactions');
            Route::post('/transactions/export', 'WalletSettingController@exportTransactions')->name('transactions.export');
            ...
            Route::prefix('invoices')->group(function () {
                Route::get('/', 'PaymentController@invoices')->name('invoices');
                Route::post('export', 'PaymentController@exportInvoice')->name('invoices.export');
                Route::post('/instructor/order/type', 'PaymentController@instructorOrderType')->name('instructor.order.type');
            });

            Route::get('full-payments', 'PaymentController@fullPayment')->name('full.payments');
            Route::get('full-payment/view/{id}', 'PaymentController@viewInvoice')->name('view.invoice');

            Route::get('installments', 'PaymentController@allInstallments')->name('installment.invoice');
            Route::get('installment-invoice/view/{id}', 'PaymentController@installmentInvoice')->name('view.installment.invoice');
            // Route::get('pending-installments', 'PaymentController@pendingInstallments');
            ...
            Route::get('about_us', 'TermsController@about_us')->name('about_us');
            // Route::put('about_us', 'TermsController@updateAboutUs');
            ...
            Route::resource('payment-charges', 'PaymentGatewayController');
            ...
            Route::middleware(['is_admin'])->group(function () {
                ...
                Route::get('/fingerprints/{email}', 'UserController@userFingerprint');
                ...
                Route::get('enrollments', 'ManualEnrollmentController@index')->name('order.enrollments');

                Route::prefix('manual')->group(function () {
                    Route::get('enrollment', 'ManualEnrollmentController@create')->name('manual.enrollment.create');
                    Route::post('enrollment', 'ManualEnrollmentController@store')->name('manual.enrollment.store');
                    Route::get('enrollment/installments/{id}', 'ManualEnrollmentController@pendingInstallments')->name('manual.enrollment.installment');
                    Route::get('enrollment/view/{id}', 'ManualEnrollmentController@view')->name('manual.enrollment.view');
                    Route::post('pay-installment/{id}', 'ManualEnrollmentController@payInstallment')->name('manual.pay.installment');
                    Route::get('enrollment/status/', 'ManualEnrollmentController@enrollmentStatus')->name('enrollment.status');
                    Route::post('enrollment/delete/{id}', 'ManualEnrollmentController@enrollmentDestroy')->name('enrollment.delete');
                    //Manual enrollment AJAX calls
                    Route::post('order/items', 'ManualEnrollmentController@orderItems')->name('order.items');
                    Route::post('coursechapters', 'ManualEnrollmentController@courseChapters')->name('course.chapters');
                    Route::post('installments', 'ManualEnrollmentController@installments')->name('item.installments');
                });

                Route::prefix('testusers')->group(function () {
                    Route::get('/', 'TestUserController@index')->name('testuser.index');
                    Route::get('/create', 'TestUserController@create')->name('testuser.create');
                    Route::post('/', 'TestUserController@store')->name('testuser.store');
                    Route::get('{id}/edit', 'TestUserController@edit')->name('testuser.edit');
                    Route::put('/{id}', 'TestUserController@update')->name('testuser.update');
                    Route::get('/{id}/enrollment', 'TestUserController@enrollment')->name('testuser.enrollments');
                    Route::get('/status', 'AlluserController@testUser')->name('testuser.status');
                });

                Route::view('/get-file', 'admin.excel.import');
                Route::post('/file-import', 'StudentOrderImportController@fileImport')->name('file-import');
                ...
                Route::post('find/course', 'CouponController@course')->name('find.course');
                ...
                Route::post('/admin/order/users', 'OneSignalNotificationController@orderUsers');
                Route::post('/admin/type/orders/', 'OneSignalNotificationController@orderTypes');
                Route::post('/admin/type/orders/users', 'OneSignalNotificationController@orderTypeUsers');
                ...
                Route::prefix('wallet')->group(function () {
                    ...
                    Route::get('/checkout', 'WalletController@view')->name('wallet.user');
                    Route::post('/topup', 'WalletController@insert')->name('wallet.topup');
                    Route::post('/removetopup', 'WalletController@deduct')->name('wallet.removetopup');
            ...
            Route::middleware(['admin_instructor',])->group(function () {
                ...
                Route::prefix('userenroll')->group(function () {
                    Route::get('/fullpayments', 'InstructorEnrollController@fullpayment')->name('user.enroll.fullpayment');
                    Route::get('/fullpayment/view/{id}', 'InstructorEnrollController@viewFullpayment')->name('user.enroll.fullpayment.view');
                    Route::get('/installments', 'InstructorEnrollController@payInInstallment')->name('user.enroll.installment');
                    Route::get('/installment/view/{id}', 'InstructorEnrollController@viewInstallment')->name('user.enroll.installment.view');
                });

                // Route::resource('userenroll', 'InstructorEnrollController');
                // Route::get('pending/enrollcourse', 'InstructorEnrollController@pendingInstallments');
                ...
                Route::post('sessionsdeleteAll', 'BulkdeleteController@sessiondeleteAll')->name('sessionsdeleteAll.bulk.delete');
                ...
                Route::get('/quickupdate-course-installment/{id}', 'CourseController@courceInstallmentStatus')->name('installment.quick.status');
        ...
        Route::middleware(['is_verified', '2fa', 'maintanance_mode'])->group(function () {
            ...
            if (env('DEFAULT_THEME') == 'classic') {
                ...
                Route::get('pages/{slug}', 'PageController@showpage')->name('page.view');
                ...
                Route::get('/download/{id}', 'DownloadController@getDownload')->name('downloadPdf')->middleware(['auth', 'twofactor']);
                ...
                Route::post('removecoupon/{id}', 'ApplyCouponController@remove')->name('remove.coupon');
            ...
            Route::get('notifications/{id}', 'NotificationController@markAsRead')->name('markAsRead');
            Route::get('delete/notifications', 'NotificationController@delete')->name('deleteNotification');
