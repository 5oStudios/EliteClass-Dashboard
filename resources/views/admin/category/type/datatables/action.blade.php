<div class="dropdown">
    ...
    <div class="dropdown-menu" aria-labelledby="CustomdropdownMenuButton1">
        @can('subcategories.edit')
            <a class="dropdown-item" href="{{ route('typecategory.edit', $id) }}"><i
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-muted">
                    {{ __('Do you really want to delete this ') }} <b>{{ __('typecategory') }}</b>
                    {{ __('? This process cannot be undone') }}
                </p>
            </div>
            <div class="modal-footer">
                <form method="post" action="{{ route('typecategory.destroy', $id) }}" class="float-right">
                    {{ csrf_field() }}
                    {{ method_field('DELETE') }}
