...
<div class="row" style="margin-bottom: 40px">
              ...
              <div class="infobar-settings-sidebar-body">
                      ...
                      <div class="col-4 text-right">
                        <div class="form-group">
                          <div class="update-password1">
                            <input type="checkbox" id="myCheck1" name="type" class="custom_toggle" {{ Request::get('type') ?  'checked' : ''}}>
                          </div>
                        </div>
                      </div>
                      <div style="{{Request::get('type') ? '' : 'display:none;'}}" id="update-password1">
                          <div class="form-group text-right col-md-12">
                            <select id ="type" name="type" class="form-control select2">
                              <option value="">{{ __('Select an Option') }}</option>
                              <option {{ (Request::get('type') == 'paid') ? 'selected' : ''}} value="paid">{{ __('Paid') }}</option>
                          <option {{ (Request::get('type') == 'free') ? 'selected' : ''}} value="free">{{ __('Free') }}</option>
                        </select>
                      </div>
                    </div>
                ...
                <div class="custom-mode-setting">
                  <div class="row align-items-center pb-3">
                    ...
                    <div class="col-4 text-right"><input type="checkbox" name="status" class="custom_toggle" {{ Request::get('status') ?  'checked' : ''}} /></div>
                  </div>
                ...
                <div class="custom-mode-setting">
                  <div class="row align-items-center pb-3">
                    <div class="col-8">
                      <h6 class="mb-0 filter">{{ _('Oldest') }}</h6>
                    </div>
                    <div class="col-4 text-right"><input type="checkbox" name="asc" class="custom_toggle" {{ Request::get('asc') ?  'checked' : ''}} /></div>
                  </div>
                ...
                <div class="custom-mode-setting">
                  <div class="row align-items-center pb-3">
                    <div class="col-8">
                      <h6 class="mb-0 filter">{{ _('Newest') }}</h6>
                    </div>
                    <div class="col-4 text-right"><input type="checkbox" name="desc" class="custom_toggle" {{ Request::get('desc') ?  'checked' : ''}} /></div>
                  </div>
                        ...
                        <div class="col-4 text-right">
                          <div class="form-group">
                            <div class="update-password">
                              <input type="checkbox" id="myCheck" name="category_id" class="custom_toggle" {{ Request::get('category_id') ?  'checked' : ''}} >
                            </div>
                          </div>
                        </div>
                        <div style="{{Request::get('category_id') ? '' : 'display:none;'}}" id="update-password">
                            <div class="form-group text-right col-md-12">
                              <select id="category" autofocus="" class="form-control select2" name="category_id">
                                <option value="">{{ __('Select Category') }}</option>
                                @foreach($categorys as $category)
            
                                <option {{ (Request::get('category_id') == $category->id) ? 'selected' : ''}} value="{{ $category->id }}">{{ $category->title }}</option>
                                @endforeach
                              </select>
  ...
  <div class="col-lg-12 mt-3 text-center">
    @if(request()->get('searchTerm'))
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div style="list-style-type: none;" class="mt-4"><a href="#" style="color:black">{{ __('Type') }} <span
                    class="button-align">
                    @if($cat->type == 1 && $cat->discount_price != 0)
                    {{ __('Paid') }}
                    @elseif(($cat->type == 0) OR ($cat->type == 1 && $cat->discount_price == 0))
                    {{ __('Free') }}
                    @endif
                  </span></a></div>
            ...
            <div class="card-footer">
              <div class="row mt-3 mb-3">
                <div class="col-1"></div>
                <div class="col-2">
                  <a href="{{ route('course.view',$cat->id) }}">
                                          ...
                                          <div class="row">
                                              ...
                                              <div class="col-lg-12">
                                                  ...
                                                  <div class="button-list mt-4 mb-3">
                                                      <i class="feather icon-mail mr-2"></i>{{ $cat->user->email }}
                                                      <i class="feather icon-phone mr-2 ml-2"></i>{{ $cat->user->mobile }}
                                                  </div>
                                                  <div class="table-responsive">
                                                      <table
                                                          ...
                                                          <tbody>
                                                            ...
                                                            @isset($cat->user->country)
                                                            <tr>
                                                              <th scope="row" class="p-1">
                                                              {{__('Category')}} :</th>
                                                              <td class="p-1">
                                                                  {{ $cat->user->country->title}}</td>
                                                            </tr>
                                                            @endisset
      ...
      <div class="form-group col-md-12 d-flex justify-content-center text-center">
        <div class="col-xs-12">
            {!! $course->render() !!}
        </div>
      </div>
    ...
    $('.status2').change(function () {
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          title: 'success', text:"{{__('Status Updated Successfully')}}", type: 'success', desktop: {
