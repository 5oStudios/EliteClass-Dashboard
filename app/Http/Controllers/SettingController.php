...
class SettingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // if ($domain == 'localhost' || $domain == '127.0.0.1' || strstr($domain, '.test') || strstr($domain, '192.168.') || strstr($domain, 'mediacity.co.in') || strstr($domain, 'castleindia.in')) {
        //     return $this->extraupdate($request);
    ...
    public function extraupdate($request)
    {
        ...
        if (Auth::user()->role == 'admin') {
            ...
            if(isset($request->login_email)){
                $setting->login_email = 1;
    
            } else{
                $setting->login_email = 0;
            }
    
            if(isset($request->login_mobile)){
                $setting->login_mobile = 1;
    
            } else{
                $setting->login_mobile = 0;
            }
