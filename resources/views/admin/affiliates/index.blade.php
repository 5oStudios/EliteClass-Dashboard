            ...
            <form class="form" action="{{ route('affiliates.update') }}" method="POST" novalidate enctype="multipart/form-data">
              ...
              <div class="form-group col-md-6">
                <label class="text-dark">{{ __('Refferal Code Length:') }}</label>
                  ...
                  <input  name="ref_length" autofocus="" type="number" class="form-control" placeholder="{{ __('Please enter Refferal code Length') }}" required="" value="{{ $affilates ? strip_tags($affilates->ref_length) : "" }}">
                
              </div>

              <div class="form-group col-md-6">
                <label class="text-dark">{{ __('Point per refferal:') }}</label>
                <input  name="point_per_referral" autofocus="" type="number" step="any"  class="form-control" placeholder="{{ __('Enter Point for per Referral') }}" value="{{ $affilates ? strip_tags($affilates->point_per_referral) : "" }}">
